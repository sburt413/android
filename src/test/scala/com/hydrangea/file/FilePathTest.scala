...
class FilePathTest extends AnyFlatSpec {
  ...
  "AbsolutePath" should "build paths from local Windows path Strings" in {
    val cWindowsSystem32: AbsolutePath = AbsolutePath.localWindowsFile("C:\\Windows\\system32")
  ...
  it should "build paths from unix path Strings" in {
    val usrBin: AbsolutePath = AbsolutePath.unixFile("/usr/bin")
  ...
  it should "create relative paths" in {
    val windowsSystem32: AbsolutePath = AbsolutePath.localWindowsFile("D:\\Windows\\system32")
    val windowsSystem32EtcHosts: AbsolutePath = AbsolutePath.localWindowsFile("D:\\Windows\\system32\\etc\\hosts")
    ...
    val halJupiterMission: AbsolutePath = AbsolutePath.windowsNetworkFile("\\\\hal9k\\Jupiter\\mission")
    val halJupiterMissionSecret: AbsolutePath = AbsolutePath.windowsNetworkFile("\\\\hal9k\\Jupiter\\mission\\secret")
    ...
    val usrLocal: AbsolutePath = AbsolutePath.unixFile("/usr/local/bin")
    val usr: AbsolutePath = AbsolutePath.unixFile("/usr")
  ...
  it should "be able to be rebased" in {
    val srcWindows: AbsolutePath = AbsolutePath.localWindowsFile("D:\\Windows\\system32")
    val srcHost: AbsolutePath = AbsolutePath.localWindowsFile("D:\\Windows\\system32\\etc\\hosts")
  ...
  it should "escape special characters" in {
    val escaped: String = AbsolutePath.unixFile("/Music/Sound & Fury/Storm ('Unleashed' Mix)").escaped
