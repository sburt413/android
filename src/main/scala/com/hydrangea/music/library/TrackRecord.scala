...
case class TrackRecord(track: Track, lastIndexed: Instant) {
  ...
  def path: VirtualPath = track.path
  def lastModified: Instant = track.lastModified
  def tag: Tag = track.tag
}

object TrackRecord {
  def apply(hash: String, path: VirtualPath, lastModified: Instant, lastIndexed: Instant, tag: Tag): TrackRecord =
    TrackRecord(Track(hash: String, path: VirtualPath, lastModified: Instant, tag), lastIndexed)
